...
#here is a file that only contains neat data, this is the first thing I sent to stacy
all_neat <-merged_List_and_MSD %>%
  filter(Dilution == 1)

write.csv(all_neat, file = "compiled_Thika_data_22Nov_16_to_19Jan17.csv", row.names = FALSE)


#this is a file that only contains the DILUTED 3 PLEX data (from 22Nov16)

diluted_3plex_22Nov <- merged_List_and_MSD %>%
  filter(Dilution != 1)
...
avgConc <- OKtoAvg %>%
  ...
  drop_na(SpID)%>%#remove the rows where the is no SpID. Not sure why it is missing, i think this has to do with SpID 9308.
  ungroup()%>%
...
#this is not plotting the values in pca$x I don't think, or at least the plot comes out different than what I did above...
library(ggfortify)
...
#or maybe I'll do classical MDS: http://www.statmethods.net/advstats/mds.html

d<-dist(matrix_for_pca)

#fit the distance structure made above and represent the data in 2d space (k = 2) and return the eigenvectors
...
text(x, y, labels = row.names(matrix_for_pca), cex=.7)
